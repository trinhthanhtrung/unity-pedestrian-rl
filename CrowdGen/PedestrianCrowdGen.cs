...
/// <summary>
/// THIS CLASS IS MODIFIED FOR INTERACTING TRAINING
/// </summary>
public class PedestrianCrowdGen : MonoBehaviour
{
    ...
    public bool randomiseRoute = true; //! MODIFIED FOR INTERACTING TRAINING
    public bool useBoidMovement = false;
    ...
    void FixedUpdate()
    {
               ...
               !isWaitingForSpawn)
        {
            ...
            if (randomiseRoute)
            {
                /// MODIFIED FOR INTERACTING TRAINING
                ...
                //}
                clonedPath.GetComponent<PedestrianCrowdPath>().pedestrianStartNode.transform.position = new Vector3(Random.Range(-5f, 5f), 0, 5f);
                clonedPath.GetComponent<PedestrianCrowdPath>().pedestrianEndNode.transform.position = new Vector3(Random.Range(-5f, 5f), 0, -5f);
                PedestrianDest[] clonedPedNodes = clonedPath.GetComponent<PedestrianCrowdPath>().pedestrianNodes;
                for (int i = 0; i < clonedPedNodes.Length; i++)
                {
                    clonedPedNodes[i].transform.position = new Vector3(Random.Range(-2.5f, 2.5f), 0, Random.Range(-2.5f, 2.5f));
                }
            }
